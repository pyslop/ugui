# [@material-symbols/svg-500](https://github.com/marella/material-symbols/tree/main/svg/500)
...
```sh
npm install @material-symbols/svg-500@latest
```

The package `@material-symbols/svg-500` includes only SVGs with weight 500. To use a different weight, replace `500` in the package name with the desired weight.
...
Material Symbols are available in three styles&mdash;`outlined`, `rounded`, and `sharp`. SVGs are available for both unfistaticlled (`FILL` 0) and filled (`FILL` 1) states with grade (`GRAD`) 0 and size (`opsz`) 48px. Other variations of grade and size are not included to keep the package size small. SVGs for each icon are located in the corresponding style directory:

```js
@material-symbols/svg-500/{style}/{icon}.svg      /* Fill 0 */
@material-symbols/svg-500/{style}/{icon}-fill.svg /* Fill 1 */
```

For example, SVGs for `face` icon are located at:

```js
@material-symbols/svg-500/outlined/face.svg      /* Fill 0 */
@material-symbols/svg-500/outlined/face-fill.svg /* Fill 1 */
@material-symbols/svg-500/rounded/face.svg       /* Fill 0 */
@material-symbols/svg-500/rounded/face-fill.svg  /* Fill 1 */
@material-symbols/svg-500/sharp/face.svg         /* Fill 0 */
@material-symbols/svg-500/sharp/face-fill.svg    /* Fill 1 */
```
...
```js
import Face from '@material-symbols/svg-500/outlined/face.svg';
...
```js
import { ReactComponent as Face } from '@material-symbols/svg-500/outlined/face.svg';
```
